...
func PageBody(pageContent ...app.UI) app.UI {
	...
	topBar := &bar.TopAppBar{Title: "go-app mdc", Fixed: false, ScrollTarget: "main-content"}

	var modeButton app.HTMLButton

	if !DesktopMode {
		modeButton = icon.MIDesktopMac.Button()
		topBar.Navigation = []app.HTMLButton{menuButton}
	} else {
		modeButton = icon.MIMobileFriendly.Button()
	}
	...
	reloadButton := icon.MIRefresh.Button().OnClick(func(ctx app.Context, e app.Event) { ctx.Reload() })
